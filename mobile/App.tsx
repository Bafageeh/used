...
export default function App() {
  ...
  const listingsPath = useMemo(() => {
    ...
    if (selectedCity !== 'الكل') params.push(`city=${encodeURIComponent(selectedCity)}`);
    return `/listings${params.length ? `?${params.join('&')}` : ''}`;
  }, [selectedCategory, query, selectedCity]);
  ...
  const visibleListings = useMemo(() => {
    ...
    return rows;
  }, [listings, minPrice, maxPrice, sortMode, nearMode, nearCoords]);
